...
apt-get install -y -q --no-install-recommends \
  curl ca-certificates make g++ sudo bash git libgeoip-dev

# Install Fluentd.
/usr/bin/curl -sSL https://toolbelt.treasuredata.com/sh/install-ubuntu-xenial-td-agent2.sh | sh

curl "http://geolite.maxmind.com/download/geoip/database/GeoLiteCity.dat.gz" | gunzip > /opt/GeoLiteCity.dat
...
td-agent-gem install --no-document fluent-plugin-elasticsearch -v 1.7.0
